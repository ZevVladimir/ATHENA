...
def calc_vel_pec(part_vel, part_dist, part_in_halo_indices, halo_indices):
    ...
    for i in range(num_halos):
        ...
        # Peculiar velocity is particle velocity - halo velocity
        #CHANGE halo_indices to halo_indices[i]
        particles_vel_pec[start_vel_pec:finish_vel_pec,:] =  part_vel[start_vel_pec:finish_vel_pec,:] - halos_vel[halo_indices[i]]

        # Get each particles scaled distance
...
def calc_rad_vel(part_dist, part_vel_pec, comps, part_in_halo_indices, halo_indices, halo_info):
    ...
    for i in range(num_halos):
        ...
        #print(rad_vel_no_hub[start_vel_phys:finish_vel_phys], print(v_hubble))
        # calculate radial velocity by adding the hubble velocity to the radial component of peculiar vel
        #CHANGE both halo_indices to halo_indices[i]
        corresponding_m200m = mass_so.R_to_M(halos_r200[i], red_shift, "200c")
        curr_halo_v200 = calc_v200(corresponding_m200m, halos_r200[i])
        particles_vel_rad[start_vel_phys:finish_vel_phys] = (rad_vel_no_hub[start_vel_phys:finish_vel_phys] + v_hubble)
        particles_vel_rad[start_vel_phys:finish_vel_phys] = particles_vel_rad[start_vel_phys:finish_vel_phys]/curr_halo_v200
        
    
        # print(i)
        #print(rad_vel_no_hub[start_vel_phys:finish_vel_phys])
        # print(v_hubble/curr_halo_v200)
        #print(halos_r200[halo_indices[i]])
...
def make_bins(num_bins, radius, radius_r200, vel_rad, curr_halos_r200): 
    print("make bin")
    ...
    for i in range(num_bins - 1):
        ...
        if indices[0].size != 0:
            ...
            med_r200 = np.median((curr_halos_r200))
...
def split_by_mass(start_nu, finish_nu, num_bins):
    ...
    # indices change: 0-1 halo 1
    # halos_by_mass_indices: 0 halo 1
    halos_by_mass_indices = np.where((peak_heights >= start_nu) & (peak_heights < finish_nu))[0] 
    print("Num halos:", halos_by_mass_indices.shape[0])
    
    halos_in_bin_indices_start = indices_change[halos_by_mass_indices]
    halos_in_bin_indices_finish = indices_change[(halos_by_mass_indices + 1)]
    
    halos_in_bin_indices = np.column_stack((halos_in_bin_indices_start,halos_in_bin_indices_finish))

    #create arrays for the all the information we need
    total_part_use = np.sum((halos_in_bin_indices_finish - halos_in_bin_indices_start))
    correspond_halo_r200 = np.zeros((halos_by_mass_indices.shape[0]), dtype = np.float32)
    ...
    for i in range(halos_in_bin_indices.shape[0]):
        finish_mass_bin += (halos_in_bin_indices[i][1] - halos_in_bin_indices[i][0])
        mass_particle_vel[start_mass_bin:finish_mass_bin] = use_particle_vel[halos_in_bin_indices[i][0]:halos_in_bin_indices[i][1]]
        mass_particle_distances[start_mass_bin:finish_mass_bin] = particle_distances[halos_in_bin_indices[i][0]:halos_in_bin_indices[i][1]]
        mass_particle_halo_radius_comp[start_mass_bin:finish_mass_bin] = particle_halo_radius_comp[halos_in_bin_indices[i][0]:halos_in_bin_indices[i][1],:]     
        
        start_mass_bin = finish_mass_bin
    
    correspond_halo_r200 = halos_r200[halos_by_mass_indices]
    mass_particles_vel_pec, mass_radius_div_r200 = calc_vel_pec(mass_particle_vel, mass_particle_distances, halos_in_bin_indices, halos_by_mass_indices)
    mass_particles_vel_rad = calc_rad_vel(mass_particle_distances, mass_particles_vel_pec, mass_particle_halo_radius_comp, halos_in_bin_indices, halos_by_mass_indices, correspond_halo_r200)
...
for i in range(num_iter):
    ...
    plt.plot(avg_vel_rad_part[:,0], avg_vel_rad_part[:,1], color = c, alpha = 0.7, label = r"${0} < \nu < {1}$".format(str(start_nu), str(finish_nu)))
    start_nu += .5

print(avg_vel_rad_hub[:,1])
plt.plot(avg_vel_rad_hub[:,0], avg_vel_rad_hub[:,1], color = "purple", alpha = 0.5, linestyle = "dashed", label = r"Hubble Flow ${0} < \nu < {1}$".format(str(start_nu), str(finish_nu)))
