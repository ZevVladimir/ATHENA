...
def create_hist_max_ptl(min_ptl, inf_radius, orb_radius, inf_rad_vel, orb_rad_vel, inf_tang_vel, orb_tang_vel, num_bins, max_radius, max_rad_vel, min_rad_vel, max_tang_vel, min_tang_vel, bin_r_vr = None, bin_r_vt = None, bin_vr_vt = None):
    ...
    np_hist4[0][np_hist4[0] < min_ptl] = min_ptl
    ...
    if np.max(np_hist3[0]) > max_ptl:
        max_ptl = np.max(np_hist3[0])
    if np.max(np_hist4[0]) > max_ptl:
        max_ptl = np.max(np_hist4[0])
    if np.max(np_hist5[0]) > max_ptl:
        max_ptl = np.max(np_hist5[0])    
    if np.max(np_hist6[0]) > max_ptl:
        max_ptl = np.max(np_hist6[0])
    
    return max_ptl, np_hist1, np_hist2, np_hist3, np_hist4, np_hist5, np_hist6
...
def plot_incorrectly_classified(correct_labels, ml_labels, radii, rad_vel, tang_vel, num_bins, start_nu, end_nu, save_location, act_orb_r_vr, act_orb_r_vt, act_orb_vr_vt, act_inf_r_vr, act_inf_r_vt, act_inf_vr_vt): 
    ...
    scaled_inf_r_vr = rotate(scaled_inf_r_vr, angle = -90)
    ...
    scaled_inf_r_vt = rotate(scaled_inf_r_vt, angle = -90)
    ...
    scaled_inf_vr_vt = rotate(scaled_inf_vr_vt, angle = -90)
    ...
    scaled_orb_r_vr = rotate(scaled_orb_r_vr, angle = -90)
    ...
    scaled_orb_r_vt = rotate(scaled_orb_r_vt, angle = -90)
    ...
    scaled_orb_vr_vt = rotate(scaled_orb_vr_vt, angle = -90)
...
def plot_radius_rad_vel_tang_vel_graphs(orb_inf, radius, radial_vel, tang_vel, correct_orb_inf, title, num_bins, start_nu, end_nu, show, save, save_location):
    ...
    ml_inf_radius = radius[np.where(orb_inf == 0)]
    ml_orb_radius = radius[np.where(orb_inf == 1)]
    ml_inf_rad_vel = radial_vel[np.where(orb_inf == 0)]
    ...
    act_orb_rad_vel = radial_vel[np.where(correct_orb_inf == 1)]
    ...
    per_err_2 = percent_error(ml_hist2[0], act_hist2[0])
    per_err_3 = percent_error(ml_hist3[0], act_hist3[0])
...
#########################################################################################################################################################
    ...
    per_err_1 = rotate(per_err_1, angle = -90)
    ...
    per_err_2 = rotate(per_err_2, angle = -90)
    ...
    per_err_3 = rotate(per_err_3, angle = -90)
    ...
    per_err_4 = rotate(per_err_4, angle = -90)
    ...
    per_err_5 = rotate(per_err_5, angle = -90)
    ...
    per_err_6 = rotate(per_err_6, angle = -90)
