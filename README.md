...
We provide our trained xgboost model as a .json file located in Paper_model/ 

To load the model it is as simple as:
```
...
## Data Requirements
You need the particle data from a GADGET simulation and the .hdf5 output file from SPARTA [2][3]. We used the Erebos suite of simulations for our work which is freely available at http://erebos.astro.umd.edu/erebos/. The snapshot files are found under snaps/ and the halo catalogues under rockstar_cats_200m_bnd. The naming scheme of of these simulations is what is expected for the code.
...
- pygadgetreader needs to be cloned and installed from this repository: https://github.com/jveitchmichaelis/pygadgetreader [5]
...
<details>
<summary>Saved Dataset Information</summary>
...
10. p_snap_info:
    ...
    4. scale_factor: Scalefactor at this snapshot (calculated from redshift with colossus [4])
...
## Kinetic Energy Cut Method

To compare our model to alternative methods of classification we provide two methods that make cuts in the phase space of ln(v^2/v_200m^2) and r/R200m both of which are outputted in the datasets generated by gen_ML_dsets.py. The first method is roughly based off of Salazar 2025 (in preparation) [6] and we provide a brief description in  our paper but refer details to his paper [5] and Section 4.2 of our paper [1]. The second method is an iteration on that method that fits a simple horizontal line to each radial bin (logarithmic bins from 0 to the max radius as implemented by SPARTA) such that all particles below the line are orbiting and all those above the line are infalling. The lines are fitted by minimizing the difference in the misclassifications of infalling and orbiting particles. See our paper for further details [1].

### simple_ke_cut.py

Use *simp_ke_calib_sims* in the same manner as *test_sims* to determine which simulation's data is used to calibrate the simple kinetic energy cut. We recommend cbol_l1000_n1024 as explained in [1]. You can adjust the parameters *n_points*, *perc*, *width*, *grad_lims*, and *r_cut_calib* to improve the calibration for your specific dataset. The provided parameters are for cbol_l1000_n1024. For more information on these parameters check out [6] for a more general example.

After running several plots will be produced displaying the fitted lines in the phase-space and comparison between the density profiles from what the kinetic energy cut lines predict and what SPARTA has. The optimal slope and y-intercepts are saved to a pickle.

### opt_ke_cut.py

For the *opt_ke_calib_sims* sims optimizes a horizontal line on a bin by bin and saves the corresponding y intercept values. Plots a distribution plot with the simple kinetic energy cut as well. Also compares the density profiles generated from this method with SPARTA

### compare_ke_plot.py

After both simple_ke_cut.py and opt_ke_cut.py are run and their values saved you can run this code to generate a density profile plot seen in [1] where the density profiles for both methods are compared against SPARTA and plotted side by side.

## Citations

[1] Vladimir Z., Osinga C., Benedikt D., Salazar, E., and Rozo, E. "Dark Matter Particle Orbiting Infalling Classification with Machine Learning". http://arxiv.org/abs/2506.09146
[2] Diemer B., 2017, The Astrophysical Journal Supplement Series, 231, 5
[3] Diemer B., 2020, The Astrophysical Journal Supplement Series, 251, 17
[4] Diemer B., 2018, The Astrophysical Journal Supplement Series, 239, 35
[5] Thompson R., 2014, pyGadgetReader: GADGET snapshot reader for python
[6] Salazar E. M., 2025, Manuscript in Prep
