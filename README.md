...
Use *simp_ps_calib_sims* in the same manner as *test_sims* to determine which simulation's data is used to calibrate the simple phase space cut. We recommend cbol_l1000_n1024 as explained in [1]. You can adjust the parameters *n_points*, *perc*, *width*, *grad_lims*, and *r_cut_calib* to improve the calibration for your specific dataset. The provided parameters are for cbol_l1000_n1024. For more information on these parameters check out [5] for a more general example.

After running several plots will be produced displaying the fitted lines in the phase-space and comparison between the density profiles from what the phase-space lines predict and what SPARTA has. The optimal slope and y-intercepts are saved to a pickle.

### opt_ps_cut.py

For the *opt_ps_calib_sims* sims optimizes a horizontal line on a bin by bin and saves the corresponding y intercept values. Plots a distribution plot with the simple phase-space cut as well. Also compares the density profiles generated from this method with SPARTA

### compare_ps_plot.py

After both simple_ps_cut.py and opt_ps_cut.py are run and their values saved you can run this code to generate a density profile plot seen in [1] where the density profiles for both methods are compared against SPARTA and plotted side by side.
