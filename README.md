...
To compare our model to alternative methods of classification we provide two methods that make cuts in the phase space of ln(v^2/v_200m^2) and r/R200m both of which are outputted in the datasets generated by gen_ML_dsets.py. The first method is roughly based off of Salazar 2025 (in preparation) [5] and we provide a brief description in  our paper but refer details to his paper [5] and Section 4.2 of our paper [1]. The second method is an iteration on that method that fits a simple horizontal line to each radial bin (logarithmic bins from 0 to the max radius as implemented by SPARTA) such that all particles below the line are orbiting and all those above the line are infalling. The lines are fitted by minimizing the difference in the misclassifications of infalling and orbiting particles. See our paper for further details [1].

### simple_ke_cut.py

Use *simp_ke_calib_sims* in the same manner as *test_sims* to determine which simulation's data is used to calibrate the simple kinetic energy cut. We recommend cbol_l1000_n1024 as explained in [1]. You can adjust the parameters *n_points*, *perc*, *width*, *grad_lims*, and *r_cut_calib* to improve the calibration for your specific dataset. The provided parameters are for cbol_l1000_n1024. For more information on these parameters check out [5] for a more general example.
