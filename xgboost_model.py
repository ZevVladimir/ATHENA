...
for snap in snapshot_list:
    ...
    if snap == snapshot_list[0]:
        start_pids = dataset_df["PIDS_" + snapshot_list[0]].to_numpy()
        for col,prop in enumerate(snap_halo_prop_names):
            halo_prop_df.assign({prop: halo_props[:,col]})
    else:
        compare_pids = dataset[:,4]
        match_pids_start, match_pids_compare = np.intersect1d(start_pids, compare_pids, return_indices=True)[1:]
        dataset[match_pids_start] = dataset[match_pids_compare]

    for col,feature in enumerate(snap_feature_names):
        if snap != snapshot_list[0] and feature == "PIDS":
            continue
        else:
            dataset_df.assign({feature: dataset[:,col]})
