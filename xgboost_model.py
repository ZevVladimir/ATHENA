...
t1 = time.time()
num_splits = 1
param_load_list = ['Orbit_Infall', 'Scaled_radii', 'Radial_vel', 'Radial_vel_magn', 'Tangential_vel', 'Tangential_vel_magn']
all_feature_names = np.array(['Scaled_radii', 'Radial_vel_x', 'Radial_vel_y', 'Radial_vel_z', 'Radial_vel_magn', 'Tangential_vel_x', 'Tangential_vel_y', 'Tangential_vel_z', 'Tangential_vel_magn'])
num_test_halos = 5
...
dataset, halo_props = build_ml_dataset(save_location, data_location, random_halo_indices, curr_snapshot, param_load_list, "full")
dataset_df = pd.DataFrame(dataset[:,1:], columns = all_feature_names)
...
if os.path.exists(save_location + "xgb_model" + curr_snapshot + ".pickle"):
    ...
    for i in range(num_splits):
        ...
        graph_feature_importance(all_feature_names, model.feature_importances_)
